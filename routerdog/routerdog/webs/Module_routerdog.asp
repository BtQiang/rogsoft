...
#routerdog_main { border:1px solid #91071f; } /* W3C rogcss */
</style>
<script>
var odm = '<% nvram_get("productid"); %>'
var lan_ipaddr = "<% nvram_get(lan_ipaddr); %>"
var params_chk = ['routerdog_enable'];
var params_inp = [];
var	refresh_flag;
var count_down;
function init() {
...
function conf2obj(){
	...
	//	}
	//}
	if(dbus["routerdog_enable"] == "1"){
		E("routerdog_console").style.display = "";
		E("routerdog_website").href = "//" + lan_ipaddr + ":9932";
	}
}
function get_status(){
	var id = parseInt(Math.random() * 100000000);
	var postData = {"id": id, "method": "routerdog_status.sh", "params":[1], "fields": ""};
	$.ajax({
		type: "POST",
		cache: false,
		url: "/_api/",
		...
		success: function(response){
			if(response.result){
				E("routerdog_status").innerHTML = response.result;
...
function save(){
	...
	//}
	E("routerdog_apply").disabled = true;
	var id = parseInt(Math.random() * 100000000);
	var postData = {"id": id, "method": "routerdog_config.sh", "params": ["web_submit"], "fields": dbus_new};
	$.ajax({
		type: "POST",
		url: "/_api/",
		data: JSON.stringify(postData),
		dataType: "json",
		success: function(response) {
			E("routerdog_apply").disabled=false;
			get_log();
		}
	});
}
...
function get_log(flag){
	...
	$.ajax({
		...
		error: function(xhr) {
			E("loading_block_title").innerHTML = "暂无routerdog狗3.0日志信息 ...";
							...
							<table width="760px" border="0" cellpadding="5" cellspacing="0" bordercolor="#6b8fa3" class="FormTitle" id="FormTitle">
								<tr>
									<td bgcolor="#4D595D" colspan="3" valign="top">
										<div>&nbsp;</div>
										<div class="formfonttitle">路由狗（Router_Dog）<lable id="routerdog_version"><lable></div>
										<div style="float:right; width:15px; height:25px;margin-top:-20px">
											<img id="return_btn" onclick="reload_Soft_Center();" align="right" style="cursor:pointer;position:absolute;margin-left:-30px;margin-top:-25px;" title="返回软件中心" src="/images/backprev.png" onMouseOver="this.src='/images/backprevclick.png'" onMouseOut="this.src='/images/backprev.png'"></img>
										</div>
										<div style="margin:10px 0 10px 5px;" class="splitLine"></div>
										<div class="SimpleNote">
											<li>路由狗（Router_Dog）支持远程管理路由器固件的各项功能，程序作者：Carseason</li>
											<li>Router_Dog 支持 安卓和路由器平台：<a href="https://www.router_dog.com" target="_blank">官方教程首页</a></li>
										</div>
										<div id="routerdog_main">
										<table width="100%" border="1" align="center" cellpadding="4" cellspacing="0" class="FormTable">
											<thead>
												<tr>
													<td colspan="2">路由狗设定</td>
												</tr>
											</thead>
											<tr id="switch_tr">
												<th>开关</th>
												<td colspan="2">
													<div class="switch_field" style="display:table-cell;float: left;">
														<label for="routerdog_enable">
															<input id="routerdog_enable" class="switch" type="checkbox" style="display: none;">
													...
													<div style="float: right;margin-top:5px;margin-right:30px;">
														...
														<!--<a type="button" href="https://raw.githubusercontent.com/koolshare/rogsoft/master/routerdog/Changelog.txt" target="_blank" class="ks_btn" style="cursor: pointer;margin-left:5px;border:none" >更新日志</a>-->
											...
											<tr id="routerdog_console" style="display:none;">
												<th>控制台</th>
												<td>
													<a type="button" id="routerdog_website" class="ks_btn" href="" target="_blank" style="border:none">路由狗控制台</a>
												</td>
											</tr>
										</table>
										</div>
										<div class="apply_gen">
											<input class="button_gen" id="routerdog_apply" onClick="save()" type="button" value="提交" />
										</div>
									</td>
								</tr>
							</table>
